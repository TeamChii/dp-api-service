...
func GetContainerByMcIdResolver(params graphql.ResolveParams) (interface{}, error) {

	var str = `{"mc_id":` + params.Args["mc_id"].(string) + `,
		"paging":{
	...
	client := &http.Client{}
	req, err := http.NewRequest("POST", config.URL_API+"/container/load-by-mc-id", bytes.NewBuffer(jsonStr))
